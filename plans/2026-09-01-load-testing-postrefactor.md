...
- **延遲大宗在「取得DB連線」這段**：取得DB連線 p99 隨 VUS 493 → 1010 → 2502ms 成長、等鎖持平。⚠️ 但「這是 pool 槽位不足」的直覺歸因被**後續 A/B 推翻**（見〈追加實驗〉）——pool 開到 600>VUS 也沒降、CPU 還有 23% idle → 主因是**爆量現開連線的建立成本**，根本天花板是**序列化吞吐**，不是連線數。
...
## 追加實驗：調大 pool + CPU 量測（2026-09-11，測「歸因」而非假設）

為了驗證「取得DB連線 2.5s 是不是因為連線數（pool cap）不夠」，做兩件事：**把 pool 調大做 A/B** + **壓測時量 CPU**。

**A/B：VUS 500 同隊，pool 100 vs 600（`max_connections` 200→700，PG 實際開到 501 條）**

| 指標 p99 | pool 100 | pool 600 |
|---|---|---|
| 取得DB連線 | 2502ms | **2261ms（幾乎沒降）** |
| 等鎖 | 736ms | **3779ms（等待 ×5）** |
| client accept | 3.26s | **4.08s（更慢）** |

**CPU（pool 100、VUS 500，mpstat 全核）**：峰值 **idle 最低 23%**（≈ 用掉 77%）→ **4 核有餘裕、非 CPU-bound**。

**結論（把三個嫌疑犯逐一排除）**：

| 嫌疑 | 判定 | 依據 |
|---|---|---|
| 連線數不夠（pool cap） | ❌ 非主因 | pool 600>500 時取得DB連線幾乎沒降 |
| CPU 飽和 | ❌ 非主因 | 峰值仍 23% idle、非 0% |
| **爆量現開連線的建立成本** | ✅ 最可能 | VUS 60（無排隊）就已 493ms；CPU 有餘裕卻仍在等 → 等的是連線供給/序列化，不是算力 |

- **取得DB連線是複合值**（pool 排隊 + 連線建立 + 同機 CPU 競爭），單一 Stopwatch 分不開；A/B + CPU 只能排除「純連線數」「CPU 飽和」，把主因收斂到**建立成本**。
- **調大 pool 反效果**：拿掉 pool cap（原本在當**入場管制**）→ 500 個全湧向序列化 advisory lock → 等鎖 736ms→3779ms、client 更慢。→ 證明**天花板是序列化吞吐，不是 pool 大小**；pool cap 是**有益的背壓**。
- **提吞吐要動臨界區（縮短鎖內工作 / 分片 / 非同步），不是動連線層**；連線層（pool/PgBouncer）只解「連線佔用/`too many clients`」，不解序列化上限。
