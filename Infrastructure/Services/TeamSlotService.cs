    ...
    public async Task<TeamSlotUpdateResult> UpdateAsync(TeamSlotUpdateRequest teamSlotUpdateRequest, bool isAdmin, ulong currentDiscordId)
    {
        ...
        foreach (var teamSlot in teamSlotUpdateRequest.TeamSlots)
        {
            ...
            if (teamSlot.Id <= 0)
            {
                ...
                // 建隊 FK 前線檢查：把不存在的 BossId/TemplateId 轉 404，不落 DB FK → 500，
                // 避免 admin 壞 id（多為 race〔範本剛被刪〕或 API 誤用）誤觸 §4「23503＝app 漏驗」告警。
                // 註：TeamSlot 沒有 Period 外鍵——期別靠 SlotDateTime 落在區間判定（見 TeamSlotRepository），
                // 故不驗 PeriodId（手動建隊的 PeriodId 預設 0 屬正常，驗它會誤擋合法建隊）。
