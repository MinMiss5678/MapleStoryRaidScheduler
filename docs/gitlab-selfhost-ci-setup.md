...
> **現況**：實際 CI 已離開 GitLab、遷到 **GitHub Actions**（自架 GitLab CE 先改用 gitlab.com 官方托管，後來整個遷移到 GitHub Actions）→ 見 `docs/e2e-testing-setup.md` 的「CI on GitHub Actions」。本文留作**自架學習 / dind 原理參考**（親手搞懂 runner 註冊、dind、network_mode、覆蓋率合併、閘控的底層），內容不再反映現況。
