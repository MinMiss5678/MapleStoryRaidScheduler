...
| [`deployment.md`](deployment.md) | **部署操作指令**（唯一路徑：本機 `deploy.ps1` / `rollout.ps1` 經 SSH tunnel 連 k3s） | 要部署到 production（首次部署、改密碼、重置 DB） |
| [`cd-deploy-setup.md`](cd-deploy-setup.md) | **部署 runbook**：rollback、migrate 失敗恢復、expand-contract 向後相容 | 要 rollback，或 migration 出事要恢復 |
